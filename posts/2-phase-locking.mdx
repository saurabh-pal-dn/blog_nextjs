...
## Why we need locking in the first place?

Lets suppose you have 2 clients making requests to a single resource and we want to avoid any race conditions from happening. As an example, let’s consider 2 users (A and B) want to read and update a variable by 1. If both of them access the variable at the same time, they will update the value simultaneously leading to a race condition. We need to impose locking to prevent the data from being corrupted.

## Why 2PL?

We have the option to use serializable isolation to prevent race condition(RC). Serialization basically means every instruction is carried one after another. Another way to express this is, that all the instructions can be ran parallelly and the end result would be same, as they are non-conflicting. This causes the DB to acure lock on each resource and not release it unless all the operations on it are commited. This leads to loss of concurrency and high latency. A single high demanding task can pause the remaining operations for a long time.
Now lets consider 2PL
...
Now that we have the basics covered, lets define 2PL
...
1. Traditional DBs don’t possess a time limit of a transaction, if one transaction has to wait for other, long queues may be formed that have to wait for several others to complete before processing.
2. Deadlocks can happen. Though DBs generally resolve DBs automatically, but any transaction terminated has to abort all its work, that is retired. If deadlocks are frequent, that results in lots of wasted work.

## A better solution?

Worry not, we have a better soultion at hand(atleast for some cases). We may describe 2PL as Pessimistic concurrency control, and serializable locking takes this to the extreme. We can opt for optimistic concurrency control in the form of serializable snapshot isolation. We allow transactions to happen anyway in the hope that everything will turn out okay. We a transaction wants to commits the DB checks whether isolation is violated, if yes the transaction is aborted and retried.
